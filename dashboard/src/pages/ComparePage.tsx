...
export default function ComparePage() {
  ...
  const updateSlot = useCallback((idx: number, partial: Partial<ConfigSlot>) => {
  ...
  const handleRun = useCallback(() => {
    ...
    mutation.mutate({ configs }, { onSuccess: (data) => setResults(data) });
  }, [slots, mutation]);
  ...
  return (
    <div>
      <h2 className="text-lg font-semibold text-gray-900 mb-4">Compare Backtests</h2>

      <div className="flex gap-4 mb-4 flex-wrap">
        {slots.map((slot, idx) => (
          <div key={idx} className="bg-gray-50 rounded-lg border border-gray-200 p-4 w-52">
            ...
            {editingIdx === idx ? (
              <div className="space-y-2">
                <select
                  ...
                  onChange={(e) => updateSlot(idx, { instrument: e.target.value as Instrument })}
                ...
                <select
                  value={slot.slMode}
                  onChange={(e) => updateSlot(idx, { slMode: e.target.value as StopLossMode })}
                  className="w-full text-xs rounded border border-gray-200 px-2 py-1"
                >
                ...
                {slot.slMode === "FixedPoints" && (
                  <input
                    ...
                    onChange={(e) => updateSlot(idx, { slFixedPoints: e.target.value })}
                ...
                <select
                  value={slot.exitMode}
                  onChange={(e) => updateSlot(idx, { exitMode: e.target.value as ExitMode })}
                  className="w-full text-xs rounded border border-gray-200 px-2 py-1"
                >
              ...
              <>
                <p className="text-xs text-gray-600">{INSTRUMENT_MAP[slot.instrument]}</p>
                <p className="text-xs text-gray-600">SL: {slot.slMode}</p>
                {slot.slMode === "FixedPoints" && (
                  <p className="text-xs text-gray-600 font-mono">{slot.slFixedPoints} pts</p>
                )}
        ...
        <div className="space-y-6">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-200">
                  <th className="text-left py-2 px-3 text-xs font-medium text-gray-500">Metric</th>
                  {results.map((_, idx) => (
                    <th
                      key={idx}
              ...
              <tbody className="font-mono tabular-nums text-gray-700">
                {[
                  {
                    label: "Trades",
                    get: (r: CompareResultItem) => String(r.result.stats.total_trades),
                  },
                  {
                    label: "Win Rate",
                    get: (r: CompareResultItem) => `${r.result.stats.win_rate.toFixed(1)}%`,
                  },
                  {
                    label: "Profit Factor",
                  ...
                  {
                    label: "Sharpe",
                    get: (r: CompareResultItem) => r.result.stats.sharpe_ratio.toFixed(2),
                  },
                  ...
                  <tr key={row.label} className="border-b border-gray-100">
                    <td className="py-1.5 px-3 font-sans text-gray-600">{row.label}</td>
                    {results.map((r, idx) => (
          ...
          {equityData && (
            <div className="bg-gray-50 rounded-lg border border-gray-200 p-4">
              <h3 className="text-sm font-medium text-gray-700 mb-3">Overlaid Equity Curves</h3>
